...
func TestStaticPattern(t *testing.T) {
	...
	for _, tt := range tests {
		if pattern := NewPattern(tt); pattern.kind != STATIC_PATTERN && pattern.raw == tt {
...
func TestOptionalParamPattern(t *testing.T) {
	tests := []string{
		"{user?}", "{id?}",
	}
	...
	for _, tt := range tests {
		if pattern := NewPattern(tt); pattern.kind != PARAM_PATTERN {
			t.Errorf("%s was not compiled as param pattern!", tt)
			if len(pattern.wildcards) == 0 || pattern.wildcards[0] != tt[1:len(tt)-2] {
				t.Errorf("Wildcard `%s` for pattern `%s` not found or not matching!", tt[1:len(tt)], tt)
			}
			if pattern.isOptional == false {
				t.Errorf("%s was not compiled as optional pattern!", tt)
			}
		}
	}
}
